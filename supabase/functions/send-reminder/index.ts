...
serve(async (req) => {
  ...
  try {
    ...
    // Handle both JSON and edge Function JSON body
...
Balas "OK" untuk konfirmasi. Sampai jumpa! 😊`
    ...
    if (channel === 'email' && resendApiKey && patient.email) {
      ...
      // Placeholder for WhatsApp - would integrate with 360dialog
      ...
      await supabase.from('communication_log').insert({
        ...
        recipient,
        message,
        status: 'sent', // Placeholder shows as sent
      ...
      // Placeholder for SMS - would integrate with Zenziva/Twilio
